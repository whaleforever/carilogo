...
def extract_feature(db_dir, output=None, bins=(8, 12, 3), training=False):
    bins = (8, 12, 3) if not bins else bins
    ...
    for imagePath in glob.glob(db_dir + "/*"):
        ...
        # describe the image
        features = cd.describe(image, use_segments=False)
        result = []
        # convert to string
    ...
    if training:
        header = "label,%s\n" % ','.join(
            [str(a) for a in range(feature_length)])
        output.write(header)
    output.write(content)


def indexing(db_dir, bins=None, filename='file_index.csv', **kwargs):
    # open the output index file for writing
    output = open(filename, "w")
    extract_feature(db_dir, output, bins=bins, training=True, **kwargs)
...
if __name__ == "__main__":
    ...
    ap.add_argument("-t", "--training", type=bool, default=False)
    args = vars(ap.parse_args())

    indexing(args['dataset'], bins=(8, 8, 8), filename=args['index'], training=True)
